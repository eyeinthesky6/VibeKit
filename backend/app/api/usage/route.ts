import type { UsageResponse, UsageRecord } from '@/shared/types';
...
export async function GET(req: NextRequest) {
  ...
  if (startDate) filters.push(gte(usage.timestamp, new Date(startDate)));
  if (endDate) filters.push(lte(usage.timestamp, new Date(endDate)));
  ...
  return NextResponse.json<UsageResponse & { page: number; limit: number }>({
    usage: usageRecords,
    page,
    limit,
  });
