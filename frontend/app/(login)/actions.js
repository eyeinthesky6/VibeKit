...
const validatedAction = (schema, callback) => {
    return async (formData) => {
        ...
        return callback(result.data, formData);
    };
};
// API call helpers
...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
    ...
    try {
        ...
        if (redirectTo === 'checkout') {
            ...
            const checkoutResult = await apiPost('stripe/checkout', { planId: priceId });
...
export async function inviteTeamMember(formData) {
    ...
    try {
        const result = await apiPost('teams/invite', { email });
        if (result.error) {
