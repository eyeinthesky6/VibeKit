...
import { users, teams, team_members, activity_logs, ActivityType, invitations, } from '@/lib/db/schema';
...
async function logActivity(team_id, user_id, type, ip_address) {
...
export const inviteTeamMember = validatedActionWithUser(inviteTeamMemberSchema, async (data, _, user) => {
    ...
    const existingInvitation = await db
        .select()
        .from(invitations)
        .where(and(eq(invitations.email, email), eq(invitations.team_id, userWithTeam.team_id), eq(invitations.status, 'pending')))
        .limit(1);
